...
MVP должен быть не техническим отчётом, а понятной страницей ответа на вопрос: “что сейчас происходит с моим заявлением в МАИ?”. Страница группируется по крупным вопросам, чтобы не превращать результат в стену текста.
...
### Если Номер Найден: Крупная Структура

Порядок блоков:

1. Главный ответ.
2. Направления.
3. Что может измениться до дедлайна.
4. История.
5. Короткое пояснение модели.

После дедлайна приёма документов прогнозный блок становится ненужным для пользователя и не показывается в основном интерфейсе.

Дедлайн для MVP:

`25 июля 2026, 17:00 МСК`
...
- сколько осталось до завершения приёма документов, если дедлайн ещё не наступил;
...
После дедлайна в главном ответе показываем:

- `Приём документов завершён.`
- дату обновления данных;
- найденные направления;
- фактическое положение по последним данным.
...
- сценарный вывод — ниже и спокойнее, только до дедлайна;
- подробное объяснение — раскрывается по клику, чтобы не перегружать страницу.

### Что Может Измениться До Дедлайна

Этот блок показывается только до `25 июля 2026, 17:00 МСК`.

Главная идея: прогноз строится на наблюдаемой статистике последних дней, особенно на темпе появления согласий у людей выше в списке.

Показываем:

- сколько людей выше без согласия;
- сколько людей выше подали согласие за последние дни;
- как менялось число согласий выше;
- что будет, если темп сохранится;
- что будет, если темп ускорится;
- что будет, если темп замедлится.

Monte Carlo может использоваться внутри как способ построить диапазон вариантов по историческому темпу, но не как самостоятельная “вероятность поступления”.

Пользовательские формулировки:

- `По темпу последних дней`
- `Если темп согласий сохранится`
- `Если темп согласий ускорится`
- `Если темп согласий замедлится`

После дедлайна этот блок не показывается.
...
До дедлайна прогноз показываем как набор сценариев, а не как точный процент.

Минимальный набор сценариев:

- `Если темп согласий сохранится` — продолжение наблюдаемой динамики.
- `Если темп согласий ускорится` — более осторожный сценарий.
- `Если темп согласий замедлится` — более спокойный сценарий.
...
После дедлайна сценарии не показываются в основном интерфейсе.
...
Если появится несколько дней истории и понятная калибровка, можно использовать вероятностный расчёт внутри сценариев до дедлайна. После дедлайна пользовательский прогноз не нужен.
